...
export class AdminApiComponent extends Construct {
	...
	constructor(scope: Construct, id: string, props: AdminApiComponentProps) {
		...
		const distribution = new cloudfront.Distribution(
			this,
			`${Config.appName}-api-distribution`,
			{
		...
		const videoCreateUploadSignedUrlFn = new nodejs.NodejsFunction(
			this,
			`${Config.appName}-video-create-upload-signed-url-fn`,
			{
				functionName: `${Config.appName}-video-create-upload-signed-url-fn`,
				description:
					"Lambda function to generate presigned URLs for video uploads",
		...
		const videoSetMetadataFn = new nodejs.NodejsFunction(
			this,
			`${Config.appName}-video-set-metadata-fn`,
			{
		...
		const videoSearchFn = new nodejs.NodejsFunction(
			this,
			`${Config.appName}-video-search-fn`,
			{
				functionName: `${Config.appName}-video-search-fn`,
				description:
					"Lambda function to search videos by title, description, or tags",
		...
		presignedUrlResource.addMethod(
			"POST",
			videoCreateUploadSignedUrlFnIntegration,
		);
		...
		videoSetMetadataFn.addEnvironment(
			"DISTRIBUTION_ID",
			videoStorage.distribution.distributionId,
		);
