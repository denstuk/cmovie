...
export class FargateService extends Construct {
    ...
    constructor(scope: Construct, id: string, props: FargateServiceProps) {
      ...
      const { imagePath, containerPort, environment, vpc, cloudfrontHeaderValue = 'CloudFront-Secret-Header-Value' } = props;
      ...
      new elbv2.ApplicationListenerRule(this, `${Config.appName}-alb-rule`, {
        ...
        priority: 10,
        conditions: [
          elbv2.ListenerCondition.httpHeader(cfAuthHeaderName, [cloudfrontHeaderValue])
        ],
        action: elbv2.ListenerAction.forward([fargateService.targetGroup])
      });

      // Default action for the listener is to return 403 Forbidden
      new elbv2.ApplicationListenerRule(this, `${Config.appName}-default-deny-rule`, {
        listener: fargateService.listener,
        priority: 20,
