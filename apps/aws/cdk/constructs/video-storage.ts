...
export class VideoStorage extends Construct {
    ...
    onUploadProcessorFn.addToRolePolicy(
      new iam.PolicyStatement({
		...
		this.key = new cloudfront.PublicKey(
			this,
			`${Config.appName}-video-distribution-pub-key`,
			{
		...
		const keyGroup = new cloudfront.KeyGroup(
			this,
			`${Config.appName}-video-distribution-group-key`,
			{
				items: [this.key],
		...
		this.distribution = new cloudfront.Distribution(
			this,
			`${Config.appName}-video-distribution`,
			{
				defaultBehavior: {
