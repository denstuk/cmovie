...
export const ManagePage = () => {
	...
	const fetchVideos = async (reset = true) => {
		try {
			...
			if (reset) {
				// Starting fresh
				const { videos: fetchedVideos, nextToken: token } =
					await searchVideos("");
		...
		try {
			...
			setVideos((prevVideos) =>
				prevVideos.map((video) =>
					video.video_id === updatedVideoData.video_id
						? updatedVideoData
						: video,
				),
			);
			...
			<div className="container mx-auto px-4 py-16 lg:px-8">
				...
				<div className="grid grid-cols-3 gap-6 mb-8">
					...
					<div className="bg-gray-800 p-6 rounded-lg">
						...
						{isLoading ? (
							<div className="h-10 flex items-center">
								<div className="w-14 h-6 bg-gray-700 animate-pulse rounded" />
							</div>
						) : (
							<p className="text-3xl font-bold text-white">
								{metrics.averageWatchTime}
							</p>
						)}
					</div>
					<div className="bg-gray-800 p-6 rounded-lg">
						...
						{isLoading ? (
							<div className="h-10 flex items-center">
								<div className="w-12 h-6 bg-gray-700 animate-pulse rounded" />
							</div>
						) : (
							<p className="text-3xl font-bold text-white">
